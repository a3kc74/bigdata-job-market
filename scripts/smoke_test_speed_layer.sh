...
for service in kafka kafka-ui spark-master spark-worker elasticsearch kibana prometheus grafana; do
...
curl -fsS "http://localhost:9200/_cat/indices/realtime*?v" >/dev/null
curl -fsS http://localhost:8088 >/dev/null
curl -fsS http://localhost:9090/-/ready >/dev/null

echo "Speed Layer smoke test passed."

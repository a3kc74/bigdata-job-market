...
def safe_get(scraper, url, timeout=20):
    ...
    try:
        ...
        if res.status_code == 403:
            ...
            logger.error(f"[403 BLOCKED] {url}")
...
def get_total_pages(scraper):
    try:
        res = safe_get(scraper, "https://www.topcv.vn/tim-viec-lam-cong-nghe-thong-tin-cr257?category_family=r257")
        if not res:
            return []
        soup = BeautifulSoup(res.text, "html.parser")
...
def run_batch_crawler(start_page=1, end_page=3):
    ...
    for page in range(start_page, end_page + 1):
        ...
        """
        if len(links) == 0:
            logger.error(f"[-] Tìm thấy {len(links)} links ở trang {page}. Bắt đầu bóc tách...")
            with open("pages_with_error.log", "a", encoding="utf-8") as f:
                f.write(f"Không tìm thấy links ở page: {page}\n")
        """
...
"""
    input_file = "raw_jobs_batch1.jsonl"
    output_file = "raw_jobs_batch.jsonl"

    # Đọc toàn bộ dòng từ file nguồn

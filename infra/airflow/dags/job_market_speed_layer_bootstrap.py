...
with DAG(
    ...
    params={
        ...
        "run_fake_producer": Param(
            True,
            type="boolean",
            description="Run fake crawler producer after submitting the streaming job for demo data.",
        ),
    },
) as dag:
    ...
    run_fake_producer_if_requested = BashOperator(
        task_id="run_fake_producer_if_requested",
        bash_command=f"""
        set -euo pipefail

        RUN_FAKE_PRODUCER="{{{{ params.run_fake_producer }}}}"

        case "$RUN_FAKE_PRODUCER" in
          true|True|1|yes|Yes)
            echo "[airflow-speed] run_fake_producer=true, sending demo events"
            ;;
          *)
            echo "[airflow-speed] run_fake_producer=false, skipping fake producer"
            exit 0
            ;;
        esac

        kubectl delete job -n {SPARK_NAMESPACE} fake-crawler-producer --ignore-not-found=true
        kubectl apply -f {MANIFEST_ROOT}/producer/fake-crawler-producer-job.yaml

        echo "[airflow-speed] waiting for fake producer to complete"
        if ! kubectl wait \
          --for=condition=complete \
          job/fake-crawler-producer \
          -n {SPARK_NAMESPACE} \
          --timeout=300s; then

          echo "[airflow-speed] fake producer failed or timed out"
          kubectl get pods -n {SPARK_NAMESPACE} -l job-name=fake-crawler-producer
          kubectl logs -n {SPARK_NAMESPACE} -l job-name=fake-crawler-producer --tail=200 || true
          exit 1
        fi

        kubectl logs -n {SPARK_NAMESPACE} -l job-name=fake-crawler-producer --tail=100 || true
        """,
    )
    ...
    submit_speed_streaming_job >> run_fake_producer_if_requested
    run_fake_producer_if_requested >> verify_realtime_indices
